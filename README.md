# Essential Projects Meta Project
...
```
meta exec "any command" --exclude http_extension,http_contracts
meta exec "any command" --include-only http_extension,http_contracts
```
...
```
meta exec "git flow feature start my_feature" --include-only http_extension,http_contracts
```

* Starts the feature "my_feature" in the modules
`http_extension` and `http_contracts`

### 3.7 Publish a feature on multiple repositories

```
meta exec "git flow feature publish my_feature" --include-only http_extension,http_contracts
```
